class PhotosController < ApplicationController
  ...
  def create
    ...
    if @photo.save
      ...
      render json: { error: @photo.errors.full_message.join(",") }, status: 400
    end
  end
  ...
  def list
    ...
    photos = []
    Photo.where(sharing_id: sharing.id).each do |photo|
      new_photo = {
        id: photo.id,
  ...
  private
  def photo_params
    params.require(:photo).permit(:image, :sharing_id)
  end
